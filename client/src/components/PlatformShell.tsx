...
export function PlatformShell({ children }: { children: ReactNode }) {
  ...
  const unread =
    notifications.data?.filter((item) => !item.notification.isRead).length ??
    0;
    ...
    >
            ...
            {isAuthenticated ? (
              <>
                ...
                <button
                  ...
                  title={t("settings")}
                  onClick={() => setLocation("/account-center")}
                >
                  <Settings size={16} />
                </button>
                ...
                <button
                  ...
                  aria-label={t("الأجهزةوالجلسات")}
                  onClick={() =>
                    setLocation("/settings/sessions")
                  }
                >
                  <Laptop size={16} />
                </button>
      ...
      <nav className="mobile-bottom-nav sticky bottom-0 z-40 flex items-center justify-around border-t border-white/10 bg-[#160a26]/90 px-1 py-1 backdrop-blur-xl md:hidden">
        {[
          ...
          const active =
            location === href ||
            (href !== "/feed" && location.startsWith(href));
